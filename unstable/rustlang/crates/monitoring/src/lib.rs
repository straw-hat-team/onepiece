...
pub fn is_terminal(_state: &State) -> bool {
    false
}
...
pub fn decide(state: &State, command: &Command) -> Result<Vec<Event>, Error> {
    match command {
        Command::CreateMonitoring(CreateMonitoring { id, url }) => {
            ...
            Ok(vec![Event::MonitoringStarted(MonitoringStarted {
                id: id.to_string(),
                url: url.to_string(),
            })])
        }
        Command::PauseMonitoring(PauseMonitoring { id }) => {
            ...
            Ok(vec![Event::MonitoringPaused(MonitoringPaused {
                id: id.to_string(),
            })])
        }
        Command::ResumeMonitoring(ResumeMonitoring { id }) => {
            ...
            Ok(vec![Event::MonitoringResumed(MonitoringResumed {
                id: id.to_string(),
            })])
        }
    }
...
pub fn evolve(state: &State, event: &Event) -> State {
    match event {
        Event::MonitoringStarted(MonitoringStarted { id, .. }) => State {
            id: Some(id.to_string()),
            status: Status::Running,
        },
        Event::MonitoringPaused(MonitoringPaused { .. }) => State {
            status: Status::Paused,
            id: state.id.clone(),
        },
        Event::MonitoringResumed(MonitoringResumed { .. }) => State {
            status: Status::Running,
            id: state.id.clone(),
        },
    }
